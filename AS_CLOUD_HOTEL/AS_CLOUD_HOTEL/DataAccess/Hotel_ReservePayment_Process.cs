...
{
    public static class Hotel_ReservePayment_Process
    {
        public static string process(PageModel model, Int64 compid)
        {
            ...
            {
                if (VARIABLE.TABLEID == "HML_RESVPAY" && VARIABLE.TRANSDT == model.AGlMaster.TRANSDT)
                {
            ...
            //GL Process for HML_RESVPAY
            var checkDate_HML_RESVPAY = (from n in db.HmlReservePayDbSet where n.TRANSDT == model.AGlMaster.TRANSDT && n.COMPID == compid select n).OrderBy(x => x.TRANSNO).ToList();

            if (checkDate_HML_RESVPAY.Count != 0)
            {
                Int64 max_Sale = 110000;
                foreach (var get in checkDate_HML_RESVPAY)
                {
                    ...
                    if (get.TRMODE == "CASH" || get.TRMODE == "CREDIT")
                    {
                        ...
                        model.AGlMaster.REMARKS = Convert.ToString("Reserve Payment - reservation id-" + get.RESVID+"-"+ get.REMARKS);
                        model.AGlMaster.TRANSDRCR = "DEBIT";
                        model.AGlMaster.TABLEID = "HML_RESVPAY";
