...
BillApp.controller("ApiBillController", function ($scope, $http) {
    ...
    $scope.search = function (event) {
        ...
        if (companyid != "" && transMonthYear != "" && transNo != "") {
            ...
            var txtMst_SCHARGE = document.getElementById('txtMst_SCHARGE');
            var txtMst_VatAmount = document.getElementById('txtMst_VatAmount');
            var txtMst_GROSSAMT = document.getElementById('txtMst_GROSSAMT');
            var txtMst_ADVAMT = document.getElementById('txtMst_ADVAMT');
            var txtMst_DiscAmt = document.getElementById('txtMst_DiscAmt');
            var txtMst_NetAmount = document.getElementById('txtMst_NetAmount');
            var txtMst_PAIDAMT = document.getElementById('txtMst_PAIDAMT');
            var txtMst_BALAMT = document.getElementById('txtMst_BALAMT');
            var txtMst_DUEHID = document.getElementById('txtMst_DUEHID');
            var txtMst_Remarks = document.getElementById('txtMst_REMARKS_TotalBill');

            $.getJSON(
                     '/Bill/GetPaymentInfo', { "Companyid": companyid, "TransNo": transNo, "TransMonthYear": transMonthYear, "RegistrationID": registrationID },
                     function (myData) {
                         txtMst_TotalAmount.value = myData.TOTAMT;
                         txtMst_SCHARGE.value = myData.SCHARGE;
                         txtMst_VatAmount.value = myData.VATAMT;
                         txtMst_GROSSAMT.value = myData.GROSSAMT;
                         txtMst_ADVAMT.value = myData.ADVAMT;
                         txtMst_DiscAmt.value = myData.DISCOUNT;
                         txtMst_NetAmount.value = myData.NETAMT;
                         txtMst_PAIDAMT.value = myData.PAIDAMT;
                         txtMst_BALAMT.value = myData.BALAMT;
                         if (txtMst_BALAMT.value > 0) {
                             txtMst_DUEHID.value = myData.DUEHID;
                             $('#divDuehID').show();
                         }
    ...
    $scope.GetSummOfAmount = function (BillDetailsData) {
        ...
        $("#txtMst_SCHARGE").val((document.getElementById('txtMst_TotalAmount').value * (document.getElementById('txtMst_SCHARGE_RT').value / 100)).toFixed(2));
        $("#txtMst_VatAmount").val((document.getElementById('txtMst_TotalAmount').value * (document.getElementById('txtMst_VATAMT_RT').value / 100)).toFixed(2));
        $("#txtMst_GROSSAMT").val((parseFloat(document.getElementById('txtMst_TotalAmount').value) + parseFloat(document.getElementById('txtMst_SCHARGE').value) + parseFloat(document.getElementById('txtMst_VatAmount').value)).toFixed(2));
        $("#txtMst_NetAmount").val((parseFloat(document.getElementById('txtMst_GROSSAMT').value) + parseFloat(document.getElementById('txtMst_ADVAMT').value) - parseFloat(document.getElementById('txtMst_DiscAmt').value)).toFixed(2));
        $("#txtMst_BALAMT").val((parseFloat(document.getElementById('txtMst_NetAmount').value) - parseFloat(document.getElementById('txtMst_PAIDAMT').value)).toFixed(2));
        return summ;
    };
